...
- Whisper decoder primer (`_primer_for` in `whisper_service.py`): the `initial_prompt` is a base per-language string (`_PRIMERS`, EN unprimed) merged live with two data files (re-read on mtime change, no restart): `data/whisper_vocab_<lang>.txt` (proper nouns `self_eval.py` learns from corrections — it OWNS this file, regenerated from its SQLite `vocab` table, so don't hand-edit) and `data/music_vocab_<lang>.txt` (artist/band/song names so music requests transcribe right first time; hand-curated seed kept above a SENTINEL line, capped at 35 in the primer to avoid Whisper echoing). `music_vocab_en.txt` also primes English. Populate music names from the user's real Spotify (top artists/tracks) with `./venv/bin/python tools/sync_music_vocab.py` — needs the `user-top-read` scope, so re-auth once via `python spotify_access.py --auth` after the scope was added. Restart the Whisper service to pick up primer CODE changes; the data files are live.
